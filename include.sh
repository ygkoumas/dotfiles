...
do
	...
	then
		...
		# get the lines of the repo file that start with '####' or '""""'. Then modify and append them to the home file.
		...
		append_quote=$(grep "\"\"\"\"" $file | sed -e s/"\"\"\"\""/""/g -e s#CURRENT_FILE_PATH#"$repo_file"#g )
		append=$(printf "%s\n%s" "$append" "$append_quote")
